...
%% replace common TODO Our algorithm translates both source and target
programs into a common representation, \emph{decision trees}. For
source programs we store in the nodes a value derived from the 
first column of the matrix and for each variant we recur on a subtree.
We emit a terminal node whenever the matrix decomposition algorithm
stops with an empty matrix. For target programs we recur into a
subtree at every control flow statement. Symbolic execution is used to
express an interval set of possible values that the variable at the
branch condition can take Here are the decision tree for the source
and target example programs.
...
% TODO [Gabriel]: given the amount of information in the (new)
% previous section, it doesn't really make sense to keep this section
% if we do not say a bit more about pattern matrices.
